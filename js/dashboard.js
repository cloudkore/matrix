...
document.addEventListener('DOMContentLoaded', function() {
        const statusElement = document.getElementById('user-status'); // You'll need an element with this ID in your HTML
        if (!statusElement) {
            console.error("Element with ID 'user-status' not found in HTML.");
            return;
        }

        const urlParams = new URLSearchParams(window.location.search);
        const tokenFromApp = urlParams.get('firebaseIdToken');
        const appAction = urlParams.get('appAction');

        if (appAction === 'logout') {
            statusElement.textContent = 'You have been logged out from the app.';
            // Clear any user data you might have stored locally on the website
            localStorage.removeItem('appUserEmail');
            console.log('Website: Received logout action from app.');
        ...
        if (tokenFromApp) {
            console.log('Website: Received Firebase ID Token from app.');
            try {
                // Basic JWT parsing (Payload is the second part, Base64 URL encoded)
                const payloadBase64 = tokenFromApp.split('.')[1];
                const decodedPayload = JSON.parse(atob(payloadBase64.replace(/-/g, '+').replace(/_/g, '/')));

                console.log('Website: Decoded token payload:', decodedPayload);

                if (decodedPayload && decodedPayload.email) {
                    statusElement.textContent = 'Welcome (from app): ' + decodedPayload.email;
                    localStorage.setItem('appUserEmail', decodedPayload.email); // Optionally store it
                } else {
                    statusElement.textContent = 'Logged in via app (email not found in token).';
                }

                // IMPORTANT: This token is for the APP's session.
                // To actually sign into Firebase Web SDK on this site,
                // you would need a *custom token* generated by your backend (Pattern 2).
                // For now, we are just displaying info from the app's token.

            } catch (error) {
                console.error('Website: Error decoding or processing token:', error);
                statusElement.textContent = 'Error processing login from app.';
            }
            // Optional: Clean the token from the URL after processing
            // if (window.history.replaceState) {
            //     const cleanUrl = window.location.protocol + "//" + window.location.host + window.location.pathname;
            //     window.history.replaceState({ path: cleanUrl }, '', cleanUrl);
            // }
        } else {
            // No token from app, check if we stored user info previously
            const storedUserEmail = localStorage.getItem('appUserEmail');
            if (storedUserEmail) {
                statusElement.textContent = 'Previously logged in (from app) as: ' + storedUserEmail;
            } else {
                statusElement.textContent = 'Please log in via the app to see user details here.';
            }
            console.log('Website: No Firebase ID Token found in URL from app.');
        }
    });
